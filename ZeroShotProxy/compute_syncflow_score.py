...
def compute_synflow_per_weight(net, inputs, mode, gpu):
...
def compute_SYNCFLOW_score(model, resolution, batch_size, image_channel, is_3D, gpu=0):
    model.train()
    ...
    if gpu:
        ...
        input = input.cpu()
    grads_abs_list = compute_synflow_per_weight(net=model, inputs=input, mode='', gpu= gpu)
    score = 0
